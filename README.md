...
### Input
The model first parses the input files which are in the "input" folder. This files are in json format and they contain the commuting matrices (baseline, soft and full lockdown) and the population files. To change the epidemiological parameters, modify directly the main.cpp file (before running again, remember to recompile).
